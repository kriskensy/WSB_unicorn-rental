...
@section('content')
    <h2>{{ isset($reservation) ? 'Edit reservation' : 'New reservation' }}</h2>
    <form action="{{ isset($reservation) ? route('reservations.update', $reservation) : route('reservations.store') }}" method="POST">
        ...
        <div class="mb-3">
            ...
            <input type="datetime-local" name="rent_date" id="rent_date" class="form-control"
                   value="{{ old('rent_date', isset($reservation) ? $reservation->rent_date->format('Y-m-d\TH:i') : '') }}" required>
        ...
        <div class="mb-3">
            ...
            <input type="number" name="duration_hours" id="duration_hours" class="form-control"
                   value="{{ old('duration_hours', $reservation->duration_hours ?? 1) }}" min="1" max="8" required>
            @error('duration_hours') <div class="text-danger">{{ $message }}</div> @enderror
        </div>
        <button type="submit" class="btn btn-success">{{ isset($reservation) ? 'Save' : 'Book' }}</button>
